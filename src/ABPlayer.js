...
(function() {
	...
	ABP.bind = function(playerUnit, state) {
		...
		realtimeBitrateColumnHls.firstChild.lastChild.setAttribute('points', '1,21 180,21 60,22 60,1');
		...
		setInterval(function(){
			...
			if(window.hlsplayer && window.hlsplayer.mediaInfo){
				...
				if (playerStatsOn) {
					var realtimeBitrateArr=[];
					var time = (video.currentTime | 0) - 20;
					while (realtimeBitrateArr.length < 60) {
		...
		if (video.isBound !== true) {
			...
			var timeoutPause,lastClick=0, firstClick = true,
